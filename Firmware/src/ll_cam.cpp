...
static uint8_t BIT12_8[4096] = {};
static void IRAM_ATTR ll_cam_vsync_isr(void *arg)
{
    // DBG_PIN_SET(1);
    cam_obj_t *cam = (cam_obj_t *)arg;
    BaseType_t HPTaskAwoken = pdFALSE;
    uint32_t state = LCD_CAM.lc_dma_int_st.val;
    uint32_t v = LCD_CAM.lc_dma_int_st.cam_vsync_int_st;
    if (state == 0)
    {
        return;
    }
    LCD_CAM.lc_dma_int_clr.val = state;
    if (v)
    {
...
static void IRAM_ATTR ll_cam_dma_isr(void *arg)
{
    cam_obj_t *cam = (cam_obj_t *)arg;
    BaseType_t HPTaskAwoken = pdFALSE;
    int state = GDMA.channel[cam->dma_num].in.int_st.val;
    int v = GDMA.channel[cam->dma_num].in.int_st.in_suc_eof;
    if (state == 0)
    {
        return;
    }
    GDMA.channel[cam->dma_num].in.int_clr.val = state;
    if (v)
    {
...
bool IRAM_ATTR ll_cam_stop(cam_obj_t *cam)
{
    ...
    GDMA.channel[cam->dma_num].in.int_clr.in_suc_eof = 0;
...
static esp_err_t ll_cam_dma_init(cam_obj_t *cam)
{
    ...
    {
        BIT12_8[i] = (uint8_t)floorf(powf(2.0f,(log2f(i)*8.0f/12.0f)));
    }
    for (int x = (SOC_GDMA_PAIRS_PER_GROUP - 1); x >= 0; x--)
    {
        ...
        {
            cam->dma_num = x;
            // LOG_UART("DMA Channel=%d\n", cam->dma_num);
            break;
        }
        if (x == 0)
        {
            cam_deinit();
            LOG_UART("Can't found available GDMA channel");
            return ESP_FAIL;
        }
    }
    ...
    GDMA.channel[cam->dma_num].in.pri.rx_pri = 9;//rx prio 0-15
...
esp_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    // 1: CAM_VSYNC to generate in_suc_eof.
    // 0: in_suc_eof is controlled by reg_cam_rec_data_cyclelen

    LCD_CAM.cam_ctrl1.val = 0;
    LCD_CAM.cam_ctrl1.cam_rec_data_bytelen = LCD_CAM_DMA_NODE_BUFFER_MAX_SIZE - 1;
    // Cannot be assigned to 0, and it is easy to overflow
    LCD_CAM.cam_ctrl1.cam_line_int_num = 0;
    // The number of hsyncs that generate hs interrupts
    LCD_CAM.cam_ctrl1.cam_clk_inv = 0;
    LCD_CAM.cam_ctrl1.cam_vsync_filter_en = 1;
    LCD_CAM.cam_ctrl1.cam_2byte_en = config->bits>8;
...
esp_err_t ll_cam_set_pin(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    gpio_matrix_in((gpio_num_t)config->pin_pclk, CAM_PCLK_IDX, false);
    ...
    gpio_matrix_in((gpio_num_t)config->pin_vsync, CAM_V_SYNC_IDX, cam->vsync_invert);
    ...
    gpio_matrix_in((gpio_num_t)config->pin_href, CAM_H_ENABLE_IDX, false);
    ...
    if(config->bits==12)
    {
        data_pins[0] = config->pin_d0;
        data_pins[1] = config->pin_d1;
        data_pins[2] = config->pin_d2;
        data_pins[3] = config->pin_d3;
        data_pins[4] = config->pin_d4;
        data_pins[5] = config->pin_d5;
        data_pins[6] = config->pin_d6;
        data_pins[7] = config->pin_d7;
        data_pins[8] = config->pin_d8;
        data_pins[9] = config->pin_d9;
        data_pins[10] = config->pin_d10;
        data_pins[11] = config->pin_d11;
    };
...
esp_err_t ll_cam_init_isr(cam_obj_t *cam)
{
    ...
    if (ret != ESP_OK)
    {
        LOG_UART("DMA interrupt allocation of camera failed");
        return ret;
    }
    ...
    if (ret != ESP_OK)
    {
        LOG_UART("LCD_CAM interrupt allocation of camera failed");
        return ret;
    }
    return ESP_OK;
}
...
static bool ll_cam_calc_rgb_dma(cam_obj_t *cam)
{
    ...
    // Calculate DMA size
    size_t dma_buffer_size = cam->recv_size / cam->dma_bytes_per_item;
    ...
    // LOG_UART("dma_half_buffer_size: %d, %d\n", dma_half_buffer, cam->dma_bytes_per_item);
    return 1;
}

bool ll_cam_dma_sizes(cam_obj_t *cam,const camera_config_t* config)
{
    cam->dma_bytes_per_item = config->bits>8?2:1;
...
esp_err_t ll_cam_set_sample_mode(cam_obj_t *cam, uint32_t xclk_freq_hz,const camera_config_t* config)
{
    cam->in_bytes_per_pixel = config->bits>8?2:1; // camera sends YU/YV
    cam->fb_bytes_per_pixel = 1; // frame buffer stores YU/YV/RGB565
    return ESP_OK;
}
