...
uint8_t SCCB_Read(uint8_t slv_addr, uint8_t reg)
{
    ...
    i2c_cmd_link_delete(cmd);
    if(ret != ESP_OK) {
        LOG_UART( "SCCB_Read Failed addr:0x%02x, reg:0x%02x, data:0x%02x, ret:%d\n", slv_addr, reg, data, ret);
    }
    return data;
}
...
uint8_t SCCB_Write(uint8_t slv_addr, uint8_t reg, uint8_t data)
{
    ...
    i2c_cmd_link_delete(cmd);
    if(ret != ESP_OK) {
        LOG_UART( "SCCB_Write Failed addr:0x%02x, reg:0x%02x, data:0x%02x, ret:%d\n", slv_addr, reg, data, ret);
    }
